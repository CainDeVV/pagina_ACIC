...
export default function Home() {
  ...
  useEffect(() => {
    async function carregarDadosDaHome() {
      try {
        // Consumo hiper-otimizado: O Backend for Frontend (BFF) processou as 5 queries,
        // agrupou, ordenou, isolou rascunhos e nos entregou o pacote selado.
        const resposta = await homeService.buscarDestaques();
        const { slides, servicos, diretoria, eventos, noticias } = resposta.data;

        setSlides(slides || []);
        setServicos(servicos || []);
        setDiretoria(diretoria || []);
        setEventos(eventos || []);
  ...
  // O Backend já filtrou, ordenou e limitou a exatos 3 eventos e 3 notícias!
  const homeEventos = eventos;
  const homeNoticias = noticias;
